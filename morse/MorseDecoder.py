...
def process_audio_file2(fname, x, y, tone):
    ...
    if y - x < 4.0:
        ...
        ei = int(Fs * end)
        pad = np.zeros(ei - yi)
        # print("x,y, end, xi, yi, ei",x,y, end, xi, yi, ei)
...
class Model:
    ...
    def __init__(
        self, charList, config, decoderType=DecoderType.BestPath, mustRestore=False
    ):
        ...
        self.inputImgs = tf.placeholder(
            tf.float32, shape=(None, self.imgSize[0], self.imgSize[1])
        )
        ...
        self.optimizer = tf.train.RMSPropOptimizer(self.learningRate).minimize(
            self.loss
        )
    ...
    def setupCTC(self):
        ...
        self.savedCtcInput = tf.placeholder(
            tf.float32, shape=[self.maxTextLen, None, len(self.charList) + 1]
        )
    ...
    def trainBatch(self, batch):
        ...
        feedDict = {
            ...
            self.gtTexts: sparse,
            self.seqLen: [self.maxTextLen] * numBatchElements,
...
def main():
    ...
    if args.experiments:
        print("*" * 80)
        print(f"Looking for model files in {FilePaths.fnExperiments}")
        experiments = [
            f
            for f in listdir(FilePaths.fnExperiments)
            if isfile(join(FilePaths.fnExperiments, f))
        ]
        print(experiments)
    ...
    # infer text on test audio file
        ...
        start_time = datetime.datetime.now()
        model = Model(
            open(FilePaths.fnCharList).read(), config, decoderType, mustRestore=True
        )
